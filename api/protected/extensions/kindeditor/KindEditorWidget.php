...
class KindEditorWidget extends CInputWidget
{
	...
	{
		$this->id = ucfirst($this->model).'_'.$this->name;
		/** @var CClientScript $cs */
		$path = dirname(__FILE__).DS.'assets';
		$this->assetsUrl = Yii::app()->getAssetManager()->publish($path);
		...
		$kindjs = YII_DEBUG ? $this->assetsUrl.'/kindeditor.js' : $this->assetsUrl.'/kindeditor-min.js';
		$cs->registerScriptFile($kindjs, CClientScript::POS_HEAD);
	}
	...
	public function run()
	{
		$setupOptions =  CJavaScript::encode($this->items);
		$script = 'KindEditor.ready(function(K){var editor=K.create("textarea[id='.$this->id.']", '.$setupOptions.')});';
		/** @var CClientScript $cs */
		$cs = Yii::app()->getClientScript();
